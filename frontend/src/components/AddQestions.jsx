    ...
    return (
        <div className="pb-32">
            <div className="flex items-center flex-col">
                <h1 className="text-center text-4xl mt-9">{course.name}</h1>
                <h2 className="text-center text-2xl mt-4">{quiz.name} : Instructions</h2>
                <p className="w-11/12 text-justify mt-5 text-lg">{quiz.instructions}</p>
            ...
            <ul role="list" className="w-11/12 place-content-center bg-transparent border-2 border-blue-gray-200 rounded-lg">
                {quiz_questions.map((question) => (
                    ...
                    <li key={question.question_number} className="gap-x-6 py-5 pb-6 mb-7  content-center " >
                        <div>
                            <h1 className="text-2xl mb-5 ml-3 ">{question.question_number}. {question.question}</h1>
                            {question.poetential_answers.map((answer, index) => (
                                <div className="ml-10 mb-1" key={index}>
                                    <input
                                        type="checkbox"
                                        id={`question-${question.question_number}-answer-${index}`}
                                        className="appearance-none w-4 h-4 border border-light-purple rounded-full checked:bg-light-purple checked:border-transparent focus:outline-none"
                                    />
                                    <label className="ml-6" htmlFor={`question-${question.question_number}-answer-${index}`}>{answer}</label>
                                </div>
                            ))}
                        </div>
